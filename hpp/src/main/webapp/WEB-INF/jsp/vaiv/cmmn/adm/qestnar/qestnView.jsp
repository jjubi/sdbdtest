...
<form name="qestnarQestnForm" id="qestnarQestnForm" method="post" onsubmit="return false;" enctype="multipart/form-data">
	...
	<div class="container-fluid">
		<div class="card">
			<div class="card-body">
				<h1 class="h1"><c:out value="${qestnarInfo.qestnarNm }"/></h1>
...
<%-- 								<span><c:out value="${qestnInfo.qestnOrdr }"/>. </span> --%>
							...
							<c:choose>
								...
								<c:when test="${qestnInfo.qestnTy eq 'file' }">
									<div class="form-group fileArea">
										...
										<div id="egovComFileList${qestnInfo.qestnSeqNo }">
			...
			<div class="card-footer">
				<div class="right">
					<c:choose>
						<c:when test="${viewType eq 'Prface' }">
							<a class="btn btn-primary" href="javascript:;" onclick="qestnarQestnNextFn('/cmmn/adm/qestnar/${qestnarSeqNo}/selectQestnView${not empty testYn && testYn == 'Y' ? 'Y' : '' }.do');">시작</a>
							<input type="hidden" name="qestnSeqNo" value="${qestnarInfo.firstQestnSeqNo }">
						...
						<c:otherwise>
							<c:if test="${qestnInfo.qestnSeqNo ne qestnarInfo.lastQestnSeqNo }">
								<a class="btn btn-primary" href="javascript:;" onclick="qestnarQestnNextFn('/cmmn/adm/qestnar/${qestnarSeqNo}/selectQestnView${not empty testYn && testYn == 'Y' ? 'Y' : '' }.do');">다음</a>
							</c:if>
							<c:if test="${qestnInfo.qestnSeqNo eq qestnarInfo.lastQestnSeqNo}">
								<a class="btn btn-primary" href="javascript:;" onclick="qestnarQestnNextFn('/cmmn/adm/qestnar/${qestnarSeqNo}/selectQestnCnclsnView${not empty testYn && testYn == 'Y' ? 'Y' : '' }.do');">제출</a>
