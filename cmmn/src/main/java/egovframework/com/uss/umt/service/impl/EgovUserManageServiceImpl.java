...
//				entrprsManageDAO.deleteEntrprsmber(id[1]);
	...
	public String insertUser(UserManageVO userManageVO) throws SQLException, NoSuchAlgorithmException, FdlException {
		...
		if(userManageVO.getBrth() != null && !"".equals(userManageVO.getBrth())) {
			userManageVO.setBrth(userManageVO.getBrth().replaceAll("-", ""));
		}
		
		String result = userManageDAO.insertUser(userManageVO);
		
		//업무사용자 보안설정 테이블 등록
		AuthorManageScrtyestbs secVO = new AuthorManageScrtyestbs();
		secVO.setAuthorCode(userManageVO.getAuthorCode());
		secVO.setScrtyDtrmnTrgetId(uniqId);
		secVO.setMberTyCode("USR03");
		egovAuthorManageService.insertUserScrtyestbs(secVO);
...
//		userManageVO.setPassword(pass);
		...
		AuthorManageScrtyestbs secVO = new AuthorManageScrtyestbs();
		secVO.setAuthorCode(userManageVO.getAuthorCode());
		secVO.setScrtyDtrmnTrgetId(userManageVO.getUniqId());
		secVO.setMberTyCode("USR03");
		egovAuthorManageService.updateUserScrtyestbs(secVO);
	}
	...
	public int checkPassHintCnfirm(Map<String, String> map) throws SQLException {
		return userManageDAO.checkPassHintCnfirm(map);
	}
