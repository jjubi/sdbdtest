...
public class EgovSysLogServiceImpl extends EgovAbstractServiceImpl implements EgovSysLogService{
	...
	@Override
	public List<EgovMap> selectSysLogAllList() throws SQLException {
		List<EgovMap> resultList = sysLogDAO.selectSysLogAllList();
		for(EgovMap map : resultList) {
			if(map.get("errorCode") instanceof java.sql.Clob) {
				try {
					map.put("errorCode", CommonUtil.clobToString((java.sql.Clob) map.get("errorCode")));
				} catch (IOException e) {
					new Loggable().exLogging("selectSysLogAllList", e);
				}
			}
		}
	...
	@Override
	public List<EgovMap> selectSysLogSummaryList() throws SQLException {
		return sysLogDAO.selectSysLogSummaryList();
	}
