...
//		return "egovframework/com/uat/uia/EgovLoginUsr";
		return "vaiv/cmmn/adm/login/login";
	}
	...
	public String actionMain(ModelMap model, HttpServletRequest request) throws Exception {
		...
		*/
		...
		if("USR".equals(user.getUserSe())) {
			//쿠키에 있는 인증 여부 가져오기
			Map<String, String> cookieMap = CommonUtil.getCookiesToMap(request);
			if(cookieMap.get("googleOTPAuthAT") != null && cookieMap.get("googleOTPAuthAT").equals("Y")) {
				main_page = Globals.ADM_MAIN_PAGE;
			} else {
				String authInfo = EgovProperties.getProperty("Globals.twoDepthAuth.use"); 
				if(authInfo != null && authInfo.equals("GOOGLE")) {
					main_page = "redirect:/cmmn/com/cuc/getGoogleOTPAuthKey.do";
				} else {
					main_page = Globals.ADM_MAIN_PAGE;
				}
			}
...
//		request.getSession().setAttribute("loginVO", null);
		...
		return "redirect:/SecurityLogout.do";
	}
