...
<%@ taglib prefix="cctag" uri="tld/CustomTag.tld" %>
	...
	<div class="card">
		<div class="card-body">
			<h1 class="h1">${pageTitle} <spring:message code="title.list" />
				<small>(사용자 : <c:out value="${paginationInfo.totalRecordCount}"/>명)</small>
			</h1>
			<div class="table-responsive">
				<form name="listForm" action="<c:url value='/uss/umt/EgovMberManage.do'/>" method="post">
					<fieldset>
						<legend>일반회원관리 검색 영역</legend>
						<input name="selectedId" type="hidden" />
						<input name="checkedIdForDel" type="hidden" />
						<input type="hidden" name="pageIndex" value="1"/>
						<div class="ui program-search" title="일반회원관리 검색 영역">
							...
							<div class="search_inner clearfix float-right">
								<div class="fieldset">
									<div class="search-select">
										<span>
											<select class="form-control form-control-sm" name="sbscrbSttus" id="sbscrbSttus" title="<spring:message code="comUssUmt.userManageSsearch.sbscrbSttusTitle" />">
									...
									<div class="search-select">
										<span>
											<select class="form-control form-control-sm" name="searchCondition" title="<spring:message code="comUssUmt.userManageSsearch.searchConditioTitle" />">
					...
					<table class="table table-bordered text-center">
						<caption>일반회원관리 목록 : 아이디, 사용자이름, 사용자이메일, 전화번호, 등록일, 가입상태로 구성</caption>
						<colgroup>
							<col style="width: 5%;">
							<col style="width: 3%;">
							<col style="width: 15%;">
